...
local Targeting = Modules.Targeting
...
for _, player in ipairs(BootstrapPlayers:GetPlayers()) do
    CleanupCharacterMeshChams(Targeting:get_viewmodel(player))
end
...
local function GetBonePosition(viewmodel, boneName)
    local part = Targeting:getPart(viewmodel, boneName)
    if part then return part.Position end

    -- R6 fallback (exact checks, no pattern matching to avoid false positives)
    if boneName == "Head" then
        part = Targeting:getPart(viewmodel, "Head")
    elseif boneName == "UpperTorso" then
        part = Targeting:getPart(viewmodel, "Torso")
    ...
    elseif boneName == "LeftUpperArm" then
        part = Targeting:getPart(viewmodel, "Left Arm") or Targeting:getPart(viewmodel, "LeftArm")
    ...
    elseif boneName == "RightUpperArm" then
        part = Targeting:getPart(viewmodel, "Right Arm") or Targeting:getPart(viewmodel, "RightArm")
    ...
    elseif boneName == "LeftUpperLeg" then
        part = Targeting:getPart(viewmodel, "Left Leg") or Targeting:getPart(viewmodel, "LeftLeg")
    ...
    elseif boneName == "RightUpperLeg" then
        part = Targeting:getPart(viewmodel, "Right Leg") or Targeting:getPart(viewmodel, "RightLeg")
...
local function GetHealthValues(_,_,_,_, actor)
    return Targeting:get_health(actor.owner.value)
end
...
local UpdateESPObj = function(espObj, position, size, name, distanceStuds, instance, isCheap, nonHuman,
    ...
    if chamsEnabled and not isDead then
        ...
        elseif chamType == "Adornment" then
            ...
            if visCheck and shouldUpdate then
                ...
                local _, localCharacter = Targeting:get_character(LocalPlayer)
                local localViewmodel = Targeting:get_viewmodel(LocalPlayer)
                if localCharacter then table.insert(ignore, localCharacter) end
                if localViewmodel then table.insert(ignore, localViewmodel) end
        ...
        elseif chamType == "MeshChams" then
            ...
            local playerOwner = actor and actor.owner and actor.owner.value
        ...
        if rp then
            ...
            if not onVp then
                ...
                if GetCfg("OffScreenArrows.ArrowMode") == "Compass" then
                    ...
                    local char = Targeting:get_character(LocalPlayer)
                    local playerRoot = char and (
                        Targeting:getPart(instance, "HumanoidRootPart") or
                        Targeting:getPart(instance, "Torso") or
                        Targeting:getPart(instance, "BasePart")
                    )
    ...
    health, maxHealth = GetHealthValues(instance, nil, hbSource, GetCfg("HealthBar.Part"), actor)
    ...
    local leftOffset = 0
    local rightOffset = 0
    if GetCfg("HealthBar.Enabled") and instance:IsA("Model") then
    ...
    local teamOwner = actor and actor.owner and actor.owner.value or nil
    local leftTags = {}
    local rightTags = {}

    if GetCfg("TeamIndicator.Enabled") and teamOwner then
        local teamName = teamOwner:GetAttribute("Team")
        if teamName then
            local teamColor = GetCfg("TeamIndicator.UseTeamColor") and BrickColor.new(tostring(teamName)).Color or
    ...
    if teamOwner and GetCfg("FriendlyIndicator.Enabled") then
        if GetCfg("FriendlyIndicator.CheckTeam") and teamOwner:GetAttribute("Team") ~= nil and teamOwner:GetAttribute("Team") == LocalPlayer:GetAttribute("Team") then
            isFriendly = true
        end
    ...
    if GetCfg("Weapon.Enabled") then
        ...
        weaponName = Targeting:get_weapon(actor.owner.value)

        -- Fallback: the equipped weapon is a Tool under the character
        if (not weaponName or weaponName == "" or weaponName == "nil" or weaponName == "None") and GetCfg("Weapon.UseToolFallback") then
...
local Get2DBoundingBox = function(instance)
    ...
    if instance:IsA("Model") then
        rootPart = instance:FindFirstChild("HumanoidRootPart") or instance:FindFirstChild("Torso") or
            instance:FindFirstChild("torso") or instance:FindFirstChild("head") or
...
local ScanDirectories = function()
    ...
    for inst, data in pairs(newTracked) do
        if not TrackedInstances[inst] then
            ...
            TrackedInstances[inst].Actor = data.Actor
...
function ESP:Load(config)
    ...
    PlayerRemovingConnection = Players.PlayerRemoving:Connect(function(player)
        for inst, data in pairs(TrackedInstances) do
            if Targeting:get_character(player) == inst then
                data.espObj:Destroy()
                TrackedInstances[inst] = nil
            end
        end
